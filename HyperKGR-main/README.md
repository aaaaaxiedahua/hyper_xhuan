...
⏺ 举个完整的例子                                                            
  ...
  ⏺ sample vs not_sample 的区别                                               
                                                                         
  核心区别：要不要剪枝                                                      
                                                                            
  not_sample（不采样）：每一层扩展邻居后，全部保留                          
                                                                            
  第1层：东京 → 扩展出 [日本, 东京]              2个节点，全留              
  第2层：      → 扩展出 [日本, 东京, 大阪, 亚洲]  4个节点，全留
  第3层：      → 扩展出 [日本, 东京, 大阪, 亚洲, 中国, 北京, 上海...]  全留

  节点数逐层爆炸，小图没问题，大图扛不住。

  sample（采样）：每一层扩展后，只保留得分最高的 topk 个节点

  第1层：东京 → 扩展出 [日本, 东京]              2个节点，全留（不到topk）
  第2层：      → 扩展出 [日本, 东京, 大阪, 亚洲]  打分 → 只留 topk=3 个
  第3层：      → 扩展出的节点继续打分 → 只留 topk=3 个

  怎么决定留谁？

  每层有一个打分网络 W_samp，给新扩展出的节点打分：

  # models.py 中 GNNLayer
  diff_node_logit = self.W_samp(hidden_new[新节点]).squeeze(-1)
  ...
  训练时用 Gumbel-Softmax（加随机噪声的
  softmax），让采样过程可导，梯度能传回去。推理时用普通 softmax，确定性地选
  topk。

  还有一个区别：曲率

  sample 版本：    self.curvature = nn.Parameter(torch.tensor(1.0))    ←
  可学习，训练自动调
  not_sample 版本：self.curvature = torch.tensor(1e-6, requires_grad=False)
   ← 固定值，接近欧氏空间

  sample 版本让模型自己学最优的弯曲程度，not_sample 版本固定了曲率。
  ...
  │          │      not_sample      │            sample             │
  ├──────────┼──────────────────────┼───────────────────────────────┤
  │ 节点选择 │ 全部保留             │ 每层保留 topk 个              │
  ├──────────┼──────────────────────┼───────────────────────────────┤
  │ 计算量   │ 逐层爆炸             │ 可控                          │
  ├──────────┼──────────────────────┼───────────────────────────────┤
  │ 额外参数 │ 无                   │ 每层多一个 W_samp 打分网络    │
  ├──────────┼──────────────────────┼───────────────────────────────┤
  │ 曲率     │ 固定（≈0）           │ 可学习                        │
  ├──────────┼──────────────────────┼───────────────────────────────┤
  │ 适用场景 │ 小图（family、umls） │ 大图也能跑（fb15k-237、YAGO） │
  ├──────────┼──────────────────────┼───────────────────────────────┤
  │ 论文效果 │ HyperKGR             │ HyperKGR + Sample（更好）     │
  └──────────┴──────────────────────┴───────────────────────────────┘

  简单说：sample 版本 = not_sample + 智能剪枝 +
  可学习曲率，既能跑大图，效果也更好。
  ...
  0.0058, 0.9912, 0.000023, 48, 48, 3, 4, 50, 0.4770, idd  200, True      
  依次对应：lr=0.0058, decay_rate=0.9912, lambda=0.000023, hidden_dim=48, 
  init_dim=48, attn_dim=3, n_layer=4, n_batch=50, dropout=0.477, act=idd, 
  topk=200, increase=True                                   

  第 3 行：最佳 epoch 的指标（核心结果）

  格式为 time_1 time_2 time_3 | v_指标 | t_指标：

  0.5817 2.0226 2.6179 | 0.5722 216.4 0.5259 0.5831 0.6646 0.8901 | 0.7228
   33.5 0.6555 0.7614 0.8539 0.9462
  ...
  │   值   │             含义              │
  ├────────┼───────────────────────────────┤
  │ 0.5817 │ time_1：邻居采样耗时          │
  ├────────┼───────────────────────────────┤
  │ 2.0226 │ time_2：GNN 前向+反向传播耗时 │
  ├────────┼───────────────────────────────┤
  │ 2.6179 │ time_3：评估耗时              │
  └────────┴───────────────────────────────┘

  中间 6 个数 — Validation（transductive 验证集）：
  ...
  │ 0.5722 │ v_MRR        │ 平均倒数排名         │
  ├────────┼──────────────┼──────────────────────┤
  │ 216.4  │ v_MR         │ 平均排名（越小越好） │
  ├────────┼──────────────┼──────────────────────┤
  │ 0.5259 │ v_Hits@1     │ 排名第1的比例        │
  ├────────┼──────────────┼──────────────────────┤
  │ 0.5831 │ v_Hits@3     │ 排名前3的比例        │
  ├────────┼──────────────┼──────────────────────┤
  │ 0.6646 │ v_Hits@10    │ 排名前10的比例       │
  ├────────┼──────────────┼──────────────────────┤
  │ 0.8901 │ v_Hits@10/50 │ 排名前10占前50的比例 │
  └────────┴──────────────┴──────────────────────┘

  后 6 个数 — Test（inductive 测试集，核心关注）：
  ...
  │ 33.5   │ t_MR         │ 测试平均排名    │
  ├────────┼──────────────┼─────────────────┤
  │ 0.6555 │ t_Hits@1     │ 测试 Hits@1     │
  ├────────┼──────────────┼─────────────────┤
  │ 0.7614 │ t_Hits@3     │ 测试 Hits@3     │
  ├────────┼──────────────┼─────────────────┤
  │ 0.8539 │ t_Hits@10    │ 测试 Hits@10    │
  ├────────┼──────────────┼─────────────────┤
  │ 0.9462 │ t_Hits@10/50 │ 测试 Hits@10/50 │
  └────────┴──────────────┴─────────────────┘